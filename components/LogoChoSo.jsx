// Logo "Chợ Số Hồng Gai" — dùng logo mark THẬT (mái chợ + núi đá + sóng Hạ Long + chữ G),
...
export default function LogoChoSo({ size = 40, anChu = false, anChuKhiHep = false, mauChu = 'sang' }) {
    return (
        <span className='inline-flex items-center gap-2 sm:gap-2.5 select-none'>
            <img src='/thuong-hieu/logo-mark.png' alt='Chợ Số Hồng Gai'
            ...
            {!anChu && (
                ...
                <span className={`flex flex-col leading-tight ${anChuKhiHep ? 'max-[339px]:hidden' : ''}`}>
                    <span className={`text-base sm:text-xl font-bold tracking-tight ${mauChu === 'toi' ? 'text-white' : 'text-ngoc-600'}`}>Chợ Số</span>
