...
const MAU = '#059669' // xanh lá — tông của khu gian hàng chợ

// Hộp thư của CHỦ GIAN (Chợ Tươi / Quà): danh sách khách nhắn tới + khung chat (2 cột trên máy tính).
// Nằm trong khu /store nên StoreLayout đã chặn người không phải tiểu thương đã duyệt.
export default function HopThuGian() {
    const { t } = useNgonNgu()
    const [hoiThoais, setHoiThoais] = useState([])
    const [gianId, setGianId] = useState(null)
    const [chon, setChon] = useState(null)          // id hội thoại đang mở
    const [trangThai, setTrangThai] = useState('dang') // dang | ok

    useEffect(() => {
        fetch('/api/gian/tin-nhan').then(async r => {
            if (!r.ok) { setTrangThai('ok'); return }
            const d = await r.json()
            setHoiThoais(d.hoiThoais || []); setGianId(d.gianId || null); setTrangThai('ok')
    ...
    useEffect(() => {
        ...
        if (!pusher) return
        const ch = pusher.subscribe(`private-quan-${gianId}`)
        const xuLy = ({ hoiThoai }) => {
            if (!hoiThoai) return
        ...
        ch.bind('co-tin', xuLy)
        return () => { ch.unbind('co-tin', xuLy); pusher.unsubscribe(`private-quan-${gianId}`) }
    }, [gianId, chon])
                ...
                <div className={`${chon ? 'max-sm:hidden' : ''} bg-white border border-slate-100 rounded-2xl overflow-hidden divide-y divide-slate-100 h-fit`}>
                    ...
                    {hoiThoais.map(h => (
                        <button key={h.id} onClick={() => setChon(h.id)}
                            className={`w-full text-left flex items-center gap-3 px-4 py-3.5 hover:bg-slate-50 transition ${chon === h.id ? 'bg-emerald-50' : ''}`}>
